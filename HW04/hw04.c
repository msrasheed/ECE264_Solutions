...
distance (const DataPoint * datapoint, const Centroid * centroid)
{
	...
	int i;
	for (i = 0; i < centroid->dimension; i++)
	{
		sum += pow(centroid->data[i] - datapoint->data[i], 2);
	}
	return sum;
}
...
int closestCentroid (int kval, DataPoint * datapoint, Centroid * *centroids)
{
	...
	for (i = 1; i < kval; i++)
	{
		dist = distance(datapoint, centroids[i]);
		if (dist < minDist)
		{
			minDist = dist;
			mindex = i;
...
void kmean (int kval, int nval, DataPoint * *datapoints, Centroid * *centroids)
{
	...
	while (changed || iter < 1000)
	{
		changed = false;
		iter++;
		//find new closest centroid for each data point
		for (i = 0; i < nval; i++)
		{
			int closeCent = closestCentroid(kval, datapoints[i], centroids);
			if (closeCent != datapoints[i]->cluster)
			{
				datapoints[i]->cluster = closeCent;
				changed = true;
			}
		}
		//reset all centroids
		...
		//add datapoints to their centroids
		for (i = 0; i < nval; i++)
		{
			Centroid_addPoint(centroids[datapoints[i]->cluster], datapoints[i]);
		}
